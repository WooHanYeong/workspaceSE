...
public class ChatServerFrame extends JFrame {
	...
	public class ServerClient extends Thread{
		...
		public void run() {
			try {
				while(true) {
				setLog("가. ServerClient :xx 로부터 데이터를 읽기위해 쓰레드대기");
				String chatStr = in.readLine();
				setLog("나. ServerClient :xx 로부터 읽은 데이터 " + chatStr);
				
				setLog("다. ServerClient :연결된모든 클라이언트에 읽은 데이터 전송. ");
	...
	/************************************
	 * 서버에연결된 클라이언트객체(Service)들을관리(Service)할클래스 
	 * - 클라이언트객체 추가 
	 * - 클라이언트객체 찾기 
	 * - 클라이언트객체 삭제 
	 * - 모든 클라이언트객체에 데이타전송
	...
	public class ServerClientService {
		...
		public void removeClient(ServerClient client) {
			...
			setLog(client.getUserId() + " 님 퇴장");
			setLog("B.ServerClientService : 현재접속자수 : " + clientList.size()+"명");
			sendBroadcasting("XX 님 퇴장");
		}
		/*
		 *모든클라이언트에 메세지전송
		 */
