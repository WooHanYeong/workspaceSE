...
public class EchoServerFrame extends JFrame {
	...
	*/ 
	public class EchoServerThread extends Thread{
			...
			try {
				...
				while(true) {
					...
					displayLog("5.EchoServerThread:클라이언트로부터읽은데이타 ["
							...
							.getHostAddress() + "(" + socket.getPort()+ "]  : " + echoStr);
					...
					displayLog("7.EchoServerThread:클라이언드와연결된 서버쪽소켓닫기 : "+ socket);
					socket.close();
				}
			} catch (Exception e) {
			}
		...
		private PrintWriter PrintWriter(OutputStreamWriter outputStreamWriter) {
