...
public class AccountServiceReturn {
	...
	public int getAccountTotBalance() {
		int totMoney = 0;
		for (int i = 0; i < accounts.length; i++) {
			totMoney +=accounts[i].getBalance();
		}
		return totMoney;
	}
	...
	public Account[] findByBalance(int balance) {
		 ...
		 */
		Account[] findAccounts = new Account[counts];
		/*
	...
	public Account[] findByName(String name) {
		...
		}System.out.println("같은계좌의 수 : " + counts);
		...
		for (int i = 0; i < accounts.length; i++) {
				if(accounts[i].getOwner().equals(name)) {
					findAccounts[counts2] = accounts[i];
					counts2++;
		}	
	}
		return findAccounts;
				...
				for (int i = 0; i < accounts.length; i++) {
					for (int j = 0; j < accounts.length; j++) {
						...
						if (accounts[j].getBalance() > accounts[j+1].getBalance()) {
							newAccount = accounts[j];
							accounts[j] = accounts[j+1];
							accounts[j+1] = newAccount;
						}	
					}
				}
			}
