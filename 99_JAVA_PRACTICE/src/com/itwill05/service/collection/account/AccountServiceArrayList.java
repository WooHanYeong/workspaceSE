...
public class AccountServiceArrayList {
	...
	*/
	...
	public int totBalance() {
		int totBalance = 0;
		for (int i = 0; i < accountList.size(); i++) {
			Account tempAccount1 = accountList.get(i);
			totBalance +=tempAccount1.getBalance();
		}
		return totBalance;
	}
	...
	public Account findByNo(int no) {
		...
		Account tempAccount2 = accountList.get(j);
		if (tempAccount2.getNo() == no ) {
			findAccount = accountList.get(j);
			}
		}
		return findAccount;
	}
	...
	public ArrayList<Account> findByBalance(int balance) {
		...
		for (int i = 0; i < accountList.size(); i++) {
			Account tempAccount3 = accountList.get(i);
			if (tempAccount3.getBalance() >= balance) {
	...
	public ArrayList<Account> findByName(String owner) {
		ArrayList<Account> findAccountList = new ArrayList<Account>();
		for (int i = 0; i < accountList.size(); i++) {
			Account tempAccount4 = accountList.get(i);
			if(tempAccount4.getOwner().equals(owner)) {
				findAccountList.add(accountList.get(i));
			}
		}
		return findAccountList;
	...
	public  Account ipGum(int no, int m) {
		for (int i = 0; i < accountList.size(); i++) {
			Account tempAccount5 = accountList.get(i);
			if(tempAccount5.getNo() == no) {
				tempAccount5.deposit(m);
			}
		}
		return null;
	}
	...
	public Account chulGum(int no, int m) {
		for (int i = 0; i < accountList.size(); i++) {
			Account tempAccount6 = accountList.get(i);
			if(tempAccount6.getNo() == no) {
				tempAccount6.withDraw(m);
			}
		}
		return null;
	}
